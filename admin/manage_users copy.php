    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            ...
            if ($user_type === 'student') {
                ...
                $stmt2 = $conn->prepare("INSERT INTO students (student_id, registration_number, first_name, last_name, course_id, year_of_study, intake_month, intake_year, status) VALUES (?, ?, ?, ?, 1, 1, 'January', YEAR(NOW()), 'active')");
                $stmt2->bind_param("isss", $user_id, $reg_no, $first_name, $last_name);
                $stmt2->execute();
            } elseif ($user_type === 'lecturer') {
                $staff_no = 'STAFF' . str_pad($user_id, 4, '0', STR_PAD_LEFT);
                $stmt2 = $conn->prepare("INSERT INTO lecturers (lecturer_id, staff_number, first_name, last_name, rank, department_id) VALUES (?, ?, ?, ?, 'Lecturer', 1)");
                $stmt2->bind_param("isss", $user_id, $staff_no, $first_name, $last_name);
                $stmt2->execute();
        ...
        if ($stmt->execute()) {
            if ($user_type === 'student') {
                ...
                if ($check->num_rows === 0) {
                    ...
                    $stmt2 = $conn->prepare("UPDATE students SET first_name=?, last_name=? WHERE student_id=?");
                ...
                if ($check->num_rows === 0) {
                    ...
                    $stmt2 = $conn->prepare("UPDATE lecturers SET first_name=?, last_name=? WHERE lecturer_id=?");
                    $stmt2->bind_param("ssi", $first_name, $last_name, $user_id);
                    $stmt2->execute();
                }
            } elseif ($user_type === 'admin') {
...
<body class="bg-gray-100 text-gray-900 p-8 min-h-screen">
    ...
    <script type="module">
        const humanConfig = {
            ...
            face: {
                enabled: true,
                detector: { rotation: false, maxDetected: 1, minConfidence: 0.3, minSize: 100 },
                mesh: { enabled: true },
        ...
        async function initializeFaceCapture(videoId, statusId, inputId, buttonId, canvasId) {
            ...
            try {
                if (!human) {
                    ...
                    human = new window.Human(humanConfig);
                    await human.load();
            ...
            const captureHandler = async () => {
                ...
                try {
                    ...
                    if (result.face && result.face.length > 0) {
                        ...
                        let snapshotResult = await human.detect(snapshotCanvas);

                        // Retry with video frame if snapshot fails to produce embedding
                        if (!snapshotResult.face || snapshotResult.face.length === 0 || !snapshotResult.face[0].embedding) {
                            console.log('Snapshot detection failed, retrying with video frame');
                            snapshotResult = result; // Fallback to video detection
                        }
                        ...
                        if (snapshotResult.face && snapshotResult.face.length > 0) {
                            ...
                            snapshotResult.face.forEach((face, index) => {
                                console.log(`Face ${index + 1} - Score: ${face.score}, Embedding: ${face.embedding ? face.embedding.slice(0, 10) + '...' : 'none'}`);
                                if (face.score >= 0.3 && Array.isArray(face.embedding) && face.embedding.length === 128) {
                                    const descriptor = Array.from(face.embedding);
                                    const isZero = descriptor.every(val => val === 0);
                                    if (!isZero) {
                                        const box = face.box;
                                        const x = Math.max(0, Math.min(canvas.width, box[0] > 1 ? box[0] : box[0] * canvas.width));
                                        const y = Math.max(0, Math.min(canvas.height, box[1] > 1 ? box[1] : box[1] * canvas.height));
                                        const width = Math.min(canvas.width - x, box[2] > 1 ? box[2] : box[2] * canvas.width);
                                        const height = Math.min(canvas.height - y, box[3] > 1 ? box[3] : box[3] * canvas.height);
                                        ctx.strokeStyle = 'green';
                                        ctx.lineWidth = 2;
                                        ctx.strokeRect(x, y, width, height);
                                        ctx.fillStyle = 'green';
                                        ctx.font = '16px Arial';
                                        ctx.fillText(`Confidence: ${(face.score * 100).toFixed(2)}%`, x, y - 10);

                                        console.log(`Face ${index + 1} Descriptor:`, descriptor.slice(0, 10));
                                        encodings.push(descriptor);
                                    } else {
                                        console.log(`Face ${index + 1} has zero descriptor`);
                                        status.innerHTML = '<span class="text-yellow-600">Face detected but encoding is invalid. Try better lighting.</span>';
                                    }
                                } else {
                                    console.log(`Face ${index + 1} has no valid embedding or low confidence: Score=${face.score}, Embedding=${face.embedding ? 'present' : 'missing'}`);
                                    status.innerHTML = '<span class="text-yellow-600">Face detected but too small or unclear. Center your face and ensure good lighting.</span>';
                                }
                            });

                            if (encodings.length >= 1) {
                                input.value = JSON.stringify(encodings);
                                status.innerHTML = `<span class="text-green-600">Captured ${encodings.length} encoding(s).</span>`;
                                console.log('Final encodings:', encodings.map(enc => enc.slice(0, 10)));
                            } else {
                                input.value = '';
                                status.innerHTML = '<span class="text-red-600">No valid face encodings captured. Ensure face is clear and well-lit.</span>';
                                console.log('No valid encodings captured');
                                snapshotTaken = false;
                            }
                        } else {
                            input.value = '';
                            status.innerHTML = '<span class="text-red-600">No faces detected in snapshot. Try again.</span>';
        ...
        function toggleModalFaceCapture(userType, modal) {
            ...
            const container = modal.querySelector('.face-capture-container');
            ...
            if (userType === 'student') {
                ...
                container.style.display = 'none';
                const input = modal.querySelector('input[name="face_encoding"]');
                input.value = '';
