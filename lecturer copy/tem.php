...
if (!isset($conn) || $conn->connect_error) {
    ...
    if ($stmt->execute()) {
        ...
        foreach ($chart_data as $row) {
            $labels[] = $row['month'];
            $total_possible = $row['total_sessions'] * ($row['student_count'] > 0 ? $row['student_count'] : 1);
            $percentage = $total_possible > 0 ? round(($row['present_count'] / $total_possible) * 100, 2) : 0.0;
            $data[] = $percentage;
        }
    } else {
        $chart_data = [];
            ...
            <?php if (!isset($visible_widgets['notifications']) || $visible_widgets['notifications'] === 'visible'): ?>
                <div class="bg-gray-800 rounded-lg shadow-lg p-6">
                    ...
                    <form id="notification-form" method="POST">
                        <div id="notifications-container" class="space-y-2 max-h-60 overflow-y-auto">
                            ...
                            <?php else: ?>
                                <?php foreach ($notifications as $notification): ?>
                                    <div class="flex items-center space-x-2 <?= $notification['is_read'] ? 'opacity-50' : '' ?>" data-notification-id="<?= $notification['notification_id'] ?>">
                                        <input type="checkbox" name="notification_ids[]" value="<?= $notification['notification_id'] ?>" class="h-4 w-4 text-theme bg-gray-700 border-gray-600 rounded focus:ring-theme" <?= $notification['is_read'] ? 'disabled' : '' ?>>
                        ...
                        <button type="button" id="mark-read-btn" class="mt-4 bg-theme text-white px-4 py-2 rounded hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-theme text-sm" disabled>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (notificationForm) {
                ...
                const updateMarkReadBtn = () => {
                    markReadBtn.disabled = !Array.from(checkboxes).some(cb => cb.checked && !cb.disabled);
                };
                checkboxes.forEach(cb => cb.addEventListener('change', updateMarkReadBtn));
                updateMarkReadBtn();
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            selectedIds.forEach(id => {
                                const checkbox = notificationForm.querySelector(`input[value="${id}"]`);
                                if (checkbox) {
                        ...
                        if (data.success && notificationsContainer) {
                            ...
                            if (data.notifications.length === 0) {
                                ...
                                notificationsContainer.querySelectorAll('input[name="notification_ids[]"]').forEach(cb => {
                                    cb.addEventListener('change', updateMarkReadBtn);
                                });
                                updateMarkReadBtn();
                            }
            ...
            const showError = (message) => {
                ...
                alert.className = 'bg-red-700 border-2 border-red-400 text-white px-4 py-3 rounded-lg mb-4 flex justify-between items-center';
                alert.innerHTML = `
