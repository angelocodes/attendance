...
if (!isset($conn) || $conn->connect_error) {
    error_log("Database connection failed in mark_notification.php: " . ($conn->connect_error ?? 'Unknown error'), 3, '../logs/errors.log');
...
if ($stmt->execute()) {
    ...
    echo json_encode(['success' => true, 'message' => "$affected_rows notification(s) marked as read."]);
} else {
    error_log("Failed to mark notifications: " . $stmt->error, 3, '../logs/errors.log');
    http_response_code(500);
